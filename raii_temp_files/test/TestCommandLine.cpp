...
class CleanTestDirectory {
    ...
    public:
    /// Constructor
    explicit CleanTestDirectory(std::string directory)
...
class DirectoryWatch {
    ...
    bool expectCreated(const std::string& filename, bool expectFile) {
        ...
        if (read(context.fd, buffer.data(), buffer.size()) < 0) {
            perror("read failed");
            return false;
        }
    ...
    public:
    /// Constructor
    DirectoryWatch(InotifyContext& context, std::string directory)
        : context(context),
          directory(std::move(directory)) {
...
class Tester {
    ...
    array<int, 2> parentToChild;
    /// Pipe used for communication from parent to child (signal parent ready)
    array<int, 2> parentReady;

    public:
    /// Constructor
    Tester() {
        if (pipe(parentToChild.data()) != 0) {
            perror("pipe failed");
            throw;
        }
    }
    ...
    /// Setup the tester
    void setup() {
        std::array<int,2> childReady;
        if (pipe(childReady.data()) < 0) {
            perror("pipe failed");
            throw;
        }

        if (pipe(parentReady.data()) < 0) {
            perror("pipe failed");
            throw;
        }
        ...
        if (!isParent()) {
            ...
            if (!ready) {
                std::cerr << "child signaled initialization error" << std::endl;
                throw;
            }
            ...
            if (ret < 0) {
                ...
                std::cerr << "child was not ready within 500ms" << std::endl;
                throw;
            }
